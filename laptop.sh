...
for k in $(seq 100 50 300)
do
  for i in $(seq 1 1 25)
  do
    echo $k agents on instance $name-random-$i
    ./lns -m $map -a $scen-$i.scen -k $k -t $timelimit -s 0 --solver=LNS --initAlgo=PP --replanAlgo=PP --neighborSize=4  --initDestoryStrategy=Collision
  done
done
for k in $(seq 100 50 300)
do
  ...
  do
    echo $k agents on instance $name-random-$i
    ./lns -m $map -a $scen-$i.scen -k $k -t $timelimit -s 0 --solver=LNS --initAlgo=PP --replanAlgo=PP --neighborSize=8  --initDestoryStrategy=Collision
  done
done
for k in $(seq 100 50 300)
do
  ...
  do
    echo $k agents on instance $name-random-$i
    ./lns -m $map -a $scen-$i.scen -k $k -t $timelimit -s 0 --solver=LNS --initAlgo=PP --replanAlgo=PP --neighborSize=32  --initDestoryStrategy=Collision
  done
done
exit
